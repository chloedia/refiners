...
from refiners.foundationals.layer_diffuse.models import LatentTransparencyOffsetEncoderRefiners, UNet1024Refiners
...
def convert_unet1024(args: argparse.Namespace) -> None:
    ...
    converter = ModelConverter(source_model=source, target_model=target, verbose=args.verbose, threshold=10)
...
def convert_offset_encoder(args: argparse.Namespace) -> None:
    ...
    converter = ModelConverter(source_model=source, target_model=target, verbose=args.verbose, skip_output_check=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Converts a Layer diffusers model to refiners.")
    parser.add_argument(
        "--unet-from",
        type=str,
        required=True,
